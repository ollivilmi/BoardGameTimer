...
export function WheelPicker({ value, min, max, onChange, label, unit }: Props) {
  ...
  // Always snap to the mathematically nearest item, ignoring scroll velocity direction.
  // This fixes the "clearly on the next item but snaps back" issue caused by snapToInterval
  // using velocity direction rather than nearest-position logic.
  function snapTo(rawY: number) {
    const idx = Math.max(0, Math.min(data.length - 1, Math.round(rawY / ITEM_H)));
      ...
      <View style={styles.pickerWrap}>
        ...
        <Animated.ScrollView
          ...
          onScrollEndDrag={e => {
            ...
            setTimeout(() => {
              if (!momentumRef.current) snapTo(y);
            }, 50);
          }}
        ...
        >
          {data.map((v, i) => {
            const opacity = scrollAnim.interpolate({
              ...
              outputRange: [0.12, 0.38, 1.0, 0.38, 0.12],
              extrapolate: 'clamp',
            });
